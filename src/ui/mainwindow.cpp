...
void MainWindow::showUnpacker() {
    stackedWidget->setCurrentIndex(2);
}

void MainWindow::showCurlBuilder() {
...
void MainWindow::clearDecoder() {
    decoderInputEdit->clear();
    decoderOutputEdit->clear();
}

void MainWindow::copyDecoderOutput() {
...
void MainWindow::clearUnpacker() {
    unpackerInputEdit->clear();
    unpackerOutputEdit->clear();
}

void MainWindow::copyUnpackerOutput() {
...
void MainWindow::addHeader() {
    ...
    if (hasIncompleteHeader()) {
        QMessageBox::information(this, "Complete Headers",
                                 "Please fill in all header fields before adding a new one.");
        return;
    }
    ...
    headerKey->addItems({"Content-Type", "Authorization", "Accept", "User-Agent", "Accept-Encoding",
                         "Accept-Language", "Cache-Control", "Connection", "Cookie", "Host",
                         "Referer", "X-Requested-With", "X-API-Key", "X-Auth-Token",
    ...
    removeButton->setStyleSheet(
        "QPushButton { background-color: #f44336; color: white; padding: 4px 8px; border: none; "
        "border-radius: 4px; font-weight: bold; min-width: 20px; max-width: 20px; }");
...
void MainWindow::updateCurlCommand() {
        ...
        for (int i = 0; i < headersWidgetLayout->count() - 1; i++) {
            ...
            if (widget) {
                ...
                if (layout && layout->count() >= 3) {
                    ...
                    if (keyCombo && valueCombo && !keyCombo->currentText().isEmpty() &&
                        !valueCombo->currentText().isEmpty()) {
                        options.headers.append(
                            {keyCombo->currentText(), valueCombo->currentText()});
                    }
                }
            }
        }
...
void MainWindow::copyCurlCommand() {
...
void MainWindow::formatJsonBody() {
...
QIcon MainWindow::createSquareIcon(const QString &text, const QColor &bgColor) {
    ...
    // Use system font instead of Arial for better macOS compatibility
...
void MainWindow::setupHomeScreen() {
    ...
    titleLabel->setStyleSheet("font-size: 48px; font-weight: bold; margin: 30px; color: #d32f2f; "
    ...
    decoderButton->setIcon(
        createSquareIcon("DECODER\n\nBase64\nHex\nROT/Caesar", QColor("#4CAF50")));
    decoderButton->setIconSize(QSize(128, 128));
    decoderButton->setFixedSize(150, 150);
    decoderButton->setStyleSheet("QPushButton { border: 2px solid #ddd; border-radius: 8px; } "
    ...
    unpackerButton->setIcon(
        createSquareIcon("UNPACKER\n\nJavaScript\nDeobfuscation", QColor("#FF9800")));
    unpackerButton->setIconSize(QSize(128, 128));
    unpackerButton->setFixedSize(150, 150);
    unpackerButton->setStyleSheet("QPushButton { border: 2px solid #ddd; border-radius: 8px; } "
                                  "QPushButton:hover { border-color: #FF9800; }");
    ...
    curlButton->setStyleSheet("QPushButton { border: 2px solid #ddd; border-radius: 8px; } "
                              "QPushButton:hover { border-color: #9C27B0; }");
...
void MainWindow::setupDecoderScreen() {
    ...
    backButton->setStyleSheet(
        "QPushButton { background-color: #666; color: white; padding: 8px 16px; border: none; "
        "border-radius: 4px; } QPushButton:hover { background-color: #555; }");
    ...
    connect(algorithmCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            [this](int index) { rotSpinBox->setVisible(index == 2); });
    ...
    decodeButton->setStyleSheet(
        ...
        "16px; border: none; border-radius: 4px; } QPushButton:hover { background-color: #45a049; "
        "}");

    QPushButton *clearButton = new QPushButton("Clear");
    clearButton->setStyleSheet("QPushButton { background-color: #f44336; color: white; "
                               "font-weight: bold; padding: 8px 16px; border: none; border-radius: "
                               "4px; } QPushButton:hover { background-color: #da190b; }");
    ...
    copyButton->setStyleSheet("QPushButton { background-color: #2196F3; color: white; font-weight: "
                              "bold; padding: 8px 16px; border: none; border-radius: 4px; } "
...
void MainWindow::setupUnpackerScreen() {
    ...
    unpackButton->setStyleSheet(
        "QPushButton { background-color: #FF9800; color: white; font-weight: bold; padding: 8px "
        "16px; border: none; border-radius: 4px; } QPushButton:hover { background-color: #F57C00; "
        "}");

    QPushButton *clearUnpackButton = new QPushButton("Clear");
    clearUnpackButton->setStyleSheet(
        "QPushButton { background-color: #f44336; color: white; font-weight: bold; padding: 8px "
        "16px; border: none; border-radius: 4px; } QPushButton:hover { background-color: #da190b; "
        "}");
    ...
    copyUnpackButton->setStyleSheet(
        "QPushButton { background-color: #2196F3; color: white; font-weight: bold; padding: 8px "
        "16px; border: none; border-radius: 4px; } QPushButton:hover { background-color: #1976D2; "
        "}");
...
void MainWindow::setupCurlBuilderScreen() {
    ...
    connect(methodCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &MainWindow::updateCurlCommand);
    ...
    connect(verboseCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &MainWindow::updateCurlCommand);
    ...
    addHeaderButton->setStyleSheet("QPushButton { background-color: #4CAF50; color: white; "
    ...
    formatJsonButton->setStyleSheet("QPushButton { background-color: #673AB7; color: white; "
...
bool MainWindow::hasIncompleteHeader() {
    if (!headersWidget)
        return false;
...
void MainWindow::updateAddHeaderButton() {
    if (!addHeaderButton)
        return;
    ...
    if (hasIncomplete) {
        addHeaderButton->setStyleSheet("QPushButton { background-color: #cccccc; color: #666666; "
                                       "padding: 6px 12px; border: none; border-radius: 4px; }");
    } else {
        addHeaderButton->setStyleSheet("QPushButton { background-color: #4CAF50; color: white; "
                                       "padding: 6px 12px; border: none; border-radius: 4px; }");
...
void MainWindow::updateHeaderValueDropdown(QWidget *headerRow, const QString &headerName) {
